...
int test_eql_parse_metadata_item() {
    ...
    bstring text = bfromcstr("[Baz(firstName=\"bar\", alias=\"big \\\"john\\\"\")] class Foo { }");
    ...
    mu_assert(biseqcstr(metadata->metadata.items[1]->metadata_item.key, "alias"), "");
    mu_assert(biseqcstr(metadata->metadata.items[1]->metadata_item.value, "big \"john\""), "");
