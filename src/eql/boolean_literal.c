...
eql_ast_node *eql_ast_boolean_literal_create(bool value)
{
    ...
    node->boolean_literal.type_ref = eql_ast_type_ref_create_cstr("Boolean");
...
{
    if(node != NULL) {
        eql_ast_type_ref_free(node->boolean_literal.type_ref);
    }
}
...
int eql_ast_boolean_literal_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_boolean_literal_create(node->boolean_literal.value);
    check_mem(clone);
