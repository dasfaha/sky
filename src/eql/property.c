...
void eql_ast_property_free(struct eql_ast_node *node)
{
    ...
    for(i=0; i<node->property.metadata_count; i++) {
        eql_ast_node_free(node->property.metadatas[i]);
...
int eql_ast_property_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_property_create(node->property.access, NULL);
    check_mem(clone);

    rc = eql_ast_node_copy(node->property.var_decl, &clone->property.var_decl);
...
//
// Returns 0 if successful, otherwise returns -1.
int eql_ast_property_add_metadata(eql_ast_node *node, eql_ast_node *metadata)
{
    // Validate.
    check(node != NULL, "Node required");
    check(node->type == EQL_AST_TYPE_PROPERTY, "Node type must be 'property'");
    check(metadata != NULL, "Metadata required");
    
    // Append metadata.
    node->property.metadata_count++;
    node->property.metadatas = realloc(node->property.metadatas, sizeof(eql_ast_node*) * node->property.metadata_count);
...
//
// Returns 0 if successful, otherwise returns -1.
int eql_ast_property_add_metadatas(eql_ast_node *node,
                                   ...
                                   unsigned int metadata_count)
{
    ...
    for(i=0; i<metadata_count; i++) {
        int rc = eql_ast_property_add_metadata(node, metadatas[i]);
...
int eql_ast_property_preprocess(eql_ast_node *node, eql_module *module)
{
    ...
    unsigned int i;
    for(i=0; i<node->property.metadata_count; i++) {
        rc = eql_ast_node_preprocess(node->property.metadatas[i], module);
        check(rc == 0, "Unable to preprocess property metadata");
    }

    // Preprocess variable declaration
    rc = eql_ast_node_preprocess(node->property.var_decl, module);
                                      ...
                                      uint32_t *count)
{
    ...
    if(node->property.var_decl != NULL) {
        rc = eql_ast_node_get_dependencies(node->property.var_decl, dependencies, count);
                                   ...
                                   uint32_t *count)
{
    ...
    if(node->property.var_decl != NULL) {
        rc = eql_ast_node_get_type_refs(node->property.var_decl, type_refs, count);
...
int eql_ast_property_validate(eql_ast_node *node, eql_module *module)
{
    ...
    unsigned int i;
    for(i=0; i<node->property.metadata_count; i++) {
        rc = eql_ast_node_validate(node->property.metadatas[i], module);
        check(rc == 0, "Unable to validate property metadata");
    }

    // Validate variable declaration
    rc = eql_ast_node_validate(node->property.var_decl, module);
...
//
// Returns 0 if successful, otherwise returns -1.
int eql_ast_property_generate_initializer(eql_ast_node *node,
                                          eql_ast_node **ret)
{
    check(node != NULL, "Node required");
    check(ret != NULL, "Return pointer required");
    ...
    eql_ast_node *var_ref = eql_ast_var_ref_create(&this_str);
    var_ref->generated = true;
    check_mem(var_ref);
    eql_ast_node *staccess = eql_ast_staccess_create(EQL_AST_STACCESS_TYPE_PROPERTY, var_ref, node->property.var_decl->var_decl.name, NULL, 0);
    staccess->generated = true;
    check_mem(staccess);
    
    // Generate value depending on the type.
    eql_ast_node *value = NULL;
    if(eql_is_builtin_type(node->property.var_decl->var_decl.type)) {
        bstring type_name = node->property.var_decl->var_decl.type->type_ref.name;
        if(biseqcstr(type_name, "Int")) {
            value = eql_ast_int_literal_create(0); check_mem(value);
        }
        else if(biseqcstr(type_name, "Float")) {
            value = eql_ast_float_literal_create(0); check_mem(value);
        }
        else if(biseqcstr(type_name, "Boolean")) {
            value = eql_ast_boolean_literal_create(false); check_mem(value);
        }
    }

    // If built-in type initializer is not found then set it to null.
    if(value == NULL) {
        value = eql_ast_null_literal_create(); check_mem(value);
    }
    value->generated = true;

    // Generate assignment.
    eql_ast_node *var_assign = eql_ast_var_assign_create(staccess, value);
