...
eql_ast_node *eql_ast_int_literal_create(int64_t value)
{
    ...
    node->int_literal.type_ref = eql_ast_type_ref_create_cstr("Int");
...
{
    if(node != NULL) {
        eql_ast_type_ref_free(node->int_literal.type_ref);
    }
}
...
int eql_ast_int_literal_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_int_literal_create(node->int_literal.value);
    check_mem(clone);
