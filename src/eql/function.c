...
int eql_ast_function_codegen_prototype(eql_ast_node *node, eql_module *module,
    LLVMValueRef *value);

int eql_ast_function_generate_external_call(eql_ast_node *node,
    eql_module *module, LLVMBasicBlockRef block);
...
void eql_ast_function_free(struct eql_ast_node *node)
{
    ...
    eql_ast_node_free(node->function.return_type);
...
int eql_ast_function_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_function_create(node->function.name, NULL, NULL, 0, NULL);
    check_mem(clone);

    rc = eql_ast_node_copy(node->function.return_type, &clone->function.return_type);
    ...
    for(i=0; i<clone->function.arg_count; i++) {
        rc = eql_ast_node_copy(node->function.args[i], &clone->function.args[i]);
        check(rc == 0, "Unable to copy arg");
        if(clone->function.args[i]) clone->function.args[i]->parent = clone;
    }

    rc = eql_ast_node_copy(node->function.body, &clone->function.body);
...
//
// Returns 0 if successful, otherwise returns -1.
int eql_ast_function_add_arg(eql_ast_node *node,
                             eql_ast_node *farg)
{
    check(node != NULL, "Node required");
    ...
    node->function.args = realloc(node->function.args, sizeof(eql_ast_node*) * node->function.arg_count);
    check_mem(node->function.args);
                             ...
                             LLVMValueRef *value)
{
    ...
    rc = eql_ast_function_codegen_prototype(node, module, &func);
    ...
    eql_ast_node *external_metadata_node = NULL;
    rc = eql_ast_function_get_external_metadata_node(node, &external_metadata_node);
    ...
    if(external_metadata_node != NULL) {
        rc = eql_ast_function_generate_external_call(node, module, block);
                                             ...
                                             LLVMBasicBlockRef block)
{
    ...
    rc = eql_ast_metadata_get_item_value(external_metadata_node, NULL, &function_name);
    ...
    for(i=0; i<node->function.arg_count; i++) {
        eql_ast_node *farg = node->function.args[i];
        bstring arg_name = farg->farg.var_decl->var_decl.name;
        
        // Retrieve LLVM reference.
        eql_ast_node *farg_var_decl = NULL;
        LLVMValueRef farg_value = NULL;
        rc = eql_module_get_variable(module, arg_name, &farg_var_decl, &farg_value);
    ...
    if(eql_ast_type_ref_is_void(node->function.return_type)) {
                                       ...
                                       LLVMValueRef *value)
{
    ...
    rc = eql_ast_function_get_qualified_name(node, &qualified_name);
    check(rc == 0, "Unable to determine fully qualified function name");
    
    // Generate the prototype with the given name.
    rc = eql_ast_function_codegen_prototype_with_name(node, module, qualified_name, value);
                                                 ...
                                                 LLVMValueRef *value)
{
    ...
    // If there is no prototype then create one.
        ...
        for(i=0; i<arg_count; i++) {
            ...
            rc = eql_ast_type_ref_get_full_name(arg->farg.var_decl->var_decl.type, &arg_type_name);
            check(rc == 0, "Unable to retrieve function arg type name");
            
            LLVMTypeRef param = NULL;
            rc = eql_module_get_type_ref(module, arg_type_name, NULL, &param);
        ...
        // Retrieve type name.
        rc = eql_ast_type_ref_get_full_name(node->function.return_type, &return_type_name);
        check(rc == 0, "Unable to retrieve full return type name");
        
        // Determine return type.
        LLVMTypeRef return_type;
        rc = eql_module_get_type_ref(module, return_type_name, NULL, &return_type);
        check(rc == 0, "Unable to determine function return type");

        if(eql_llvm_is_complex_type(return_type)) {
...
//
// Returns 0 if successful, otherwise returns -1.
int eql_ast_function_codegen_forward_decl(eql_ast_node *node,
                                          eql_module *module)
{
    int rc;
    check(node != NULL, "Node required");
...
int eql_ast_function_preprocess(eql_ast_node *node, eql_module *module)
{
    ...
    uint32_t i;
    for(i=0; i<node->function.arg_count; i++) {
        rc = eql_ast_node_preprocess(node->function.args[i], module);
        check(rc == 0, "Unable to preprocess function argument type");
    }

    // Preprocess block.
    if(node->function.body != NULL) {
        rc = eql_ast_node_preprocess(node->function.body, module);
                                          ...
                                          eql_module *module)
{
    ...
    if(body == NULL) {
        node->function.return_type = eql_ast_type_ref_create_cstr("void");
    ...
    // Otherwise find the last return statement and determine its type.
        ...
        if(freturn == NULL || freturn->freturn.value == NULL) {
            type = eql_ast_type_ref_create_cstr("void"); check_mem(type);
        }
        ...
        // Copy return type.
        rc = eql_ast_node_copy(type, &node->function.return_type);
                                                ...
                                                eql_ast_node **external_metadata_node)
{
    ...
    *external_metadata_node = NULL;
    ...
    if(node->parent != NULL && node->parent->type == EQL_AST_TYPE_METHOD) {
        ...
        rc = eql_ast_method_get_metadata_node(node->parent, &metadata_name, external_metadata_node);
                                   ...
                                   uint32_t *count)
{
    ...
    // Add return type.
    if(node->function.return_type != NULL) {
        rc = eql_ast_node_get_type_refs(node->function.return_type, type_refs, count);
    ...
    uint32_t i;
    for(i=0; i<node->function.arg_count; i++) {
        rc = eql_ast_node_get_type_refs(node->function.args[i], type_refs, count);
        check(rc == 0, "Unable to add function argument type type refs");
    }

    // Add block type refs.
    if(node->function.body != NULL) {
        rc = eql_ast_node_get_type_refs(node->function.body, type_refs, count);
                                      ...
                                      uint32_t *count)
{
    ...
    // Add return type.
    if(node->function.return_type != NULL) {
        rc = eql_ast_node_get_dependencies(node->function.return_type, dependencies, count);
    ...
    uint32_t i;
    for(i=0; i<node->function.arg_count; i++) {
        rc = eql_ast_node_get_dependencies(node->function.args[i], dependencies, count);
        check(rc == 0, "Unable to add function argument type dependency");
    }

    // Add block dependencies.
    if(node->function.body != NULL) {
        rc = eql_ast_node_get_dependencies(node->function.body, dependencies, count);
...
int eql_ast_function_validate(eql_ast_node *node, eql_module *module)
{
    ...
    // Retrieve parent.
    eql_ast_node *method = (node->parent && node->parent->type == EQL_AST_TYPE_METHOD ? node->parent : NULL);
    ...
    uint32_t i;
    for(i=0; i<node->function.arg_count; i++) {
        rc = eql_ast_node_validate(node->function.args[i], module);
        check(rc == 0, "Unable to validate function argument type");
    }

    // Validate block.
    if(node->function.body != NULL) {
        rc = eql_ast_node_validate(node->function.body, module);
...
int eql_ast_function_dump(eql_ast_node *node, bstring ret)
{
    ...
    if(node->function.return_type != NULL) {
        rc = eql_ast_node_dump(node->function.return_type, ret);
