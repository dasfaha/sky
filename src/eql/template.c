...
{
    eql_ast_template *template = malloc(sizeof(eql_ast_template));
    check_mem(template);
...
int eql_ast_template_apply(eql_ast_template *template,
                           eql_ast_node *node)
{
    int rc;
    unsigned int i, j;
    ...
    eql_ast_node **template_vars = template->class->class.template_vars;
    unsigned int template_var_count = template->class->class.template_var_count;
    eql_ast_node **subtypes = template->type_ref->type_ref.subtypes;
    ...
    rc = eql_ast_node_get_type_refs(node, &type_refs, &type_ref_count);
    ...
    for(i=0; i<type_ref_count; i++) {
        eql_ast_node *type_ref = type_refs[i];
        
        for(j=0; j<template_var_count; j++) {
            eql_ast_node *template_var = template_vars[j];
            eql_ast_node *subtype = subtypes[j];
