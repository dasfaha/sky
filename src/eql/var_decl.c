...
{
    if(node != NULL) {
        eql_ast_node_free(node->var_decl.type);
...
int eql_ast_var_decl_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_var_decl_create(NULL, node->var_decl.name, NULL);
    check_mem(clone);

    rc = eql_ast_node_copy(node->var_decl.type, &clone->var_decl.type);
    check(rc == 0, "Unable to copy type");
    if(clone->var_decl.type) clone->var_decl.type->parent = clone;
    
    rc = eql_ast_node_copy(node->var_decl.initial_value, &clone->var_decl.initial_value);
                             ...
                             LLVMValueRef *value)
{
    ...
    eql_ast_node *farg = (node->parent != NULL && node->parent->type == EQL_AST_TYPE_FARG ? node->parent : NULL);
    eql_ast_node *property = (node->parent != NULL && node->parent->type == EQL_AST_TYPE_PROPERTY ? node->parent : NULL);
    ...
    rc = eql_ast_type_ref_get_full_name(node->var_decl.type, &type_name);
    ...
    bool is_complex_type = eql_llvm_is_complex_type(type);
    ...
    if(property == NULL && farg == NULL && !eql_is_builtin_type_name(type_name)) {
                                     ...
                                     LLVMValueRef *value)
{
    ...
    if(!eql_is_builtin_type(node->var_decl.type)) {
        // Find the class.
        eql_ast_node *class = NULL;
        rc = eql_module_get_ast_class(module, node->var_decl.type->type_ref.name, &class);
        ...
        rc = eql_ast_class_get_method(class, &deconstructor_name, &method);
        ...
        if(method != NULL) {
            // Retrieve alloca.
            LLVMValueRef ptr;
            rc = eql_module_get_variable(module, node->var_decl.name, NULL, &ptr);
                                   ...
                                   uint32_t *count)
{
    ...
    // Add type.
    rc = eql_ast_node_get_type_refs(node->var_decl.type, type_refs, count);
                                      ...
                                      uint32_t *count)
{
    ...
    // Type ref.
    rc = eql_ast_node_get_dependencies(node->var_decl.type, dependencies, count);
...
int eql_ast_var_decl_dump(eql_ast_node *node, bstring ret)
{
    ...
    if(node->var_decl.type != NULL) {
        rc = eql_ast_node_dump(node->var_decl.type, ret);
