...
eql_ast_node *eql_ast_module_create(bstring name,
...
int eql_ast_module_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_module_create(node->module.name, NULL, 0, NULL);
    check_mem(clone);
    ...
    for(i=0; i<clone->module.class_count; i++) {
        rc = eql_ast_node_copy(node->module.classes[i], &clone->module.classes[i]);
        check(rc == 0, "Unable to copy class");
        if(clone->module.classes[i]) clone->module.classes[i]->parent = clone;
    }

    rc = eql_ast_node_copy(node->module.main_function, &clone->module.main_function);
...
//
// Returns 0 if successful, otherwise returns -1.
int eql_ast_module_codegen_forward_decl(eql_ast_node *node,
                                        eql_module *module)
{
    int rc;
    check(node != NULL, "Node required");
    ...
    unsigned int i;
    for(i=0; i<node->module.class_count; i++) {
        rc = eql_ast_node_codegen_forward_decl(node->module.classes[i], module);
        check(rc == 0, "Unable to codegen class forward declarations");
    }
    
    // Generate the main function if a block exists.
    if(node->module.main_function != NULL) {
        rc = eql_ast_node_codegen_forward_decl(node->module.main_function, module);
...
int eql_ast_module_preprocess(eql_ast_node *node, eql_module *module)
{
    ...
    uint32_t i;
    for(i=0; i<node->module.class_count; i++) {
        rc = eql_ast_node_preprocess(node->module.classes[i], module);
        check(rc == 0, "Unable to preprocess module class");
    }

    // Preprocess main function.
    if(node->module.main_function != NULL) {
        rc = eql_ast_node_preprocess(node->module.main_function, module);
                                      ...
                                      eql_ast_node **ret)
{
    ...
    *ret = NULL;
    ...
    unsigned int i;
    for(i=0; i<node->module.class_count; i++) {
        eql_ast_node *class = node->module.classes[i];
                                    ...
                                    uint32_t *count)
{
    ...
    uint32_t i;
    for(i=0; i<node->module.class_count; i++) {
        rc = eql_ast_node_get_type_refs(node->module.classes[i], type_refs, count);
        check(rc == 0, "Unable to add module class type refs");
    }

    // Main function type refs.
    if(node->module.main_function != NULL) {
        rc = eql_ast_node_get_type_refs(node->module.main_function, type_refs, count);
                                    ...
                                    uint32_t *count)
{
    ...
    uint32_t i;
    for(i=0; i<node->module.class_count; i++) {
        rc = eql_ast_node_get_dependencies(node->module.classes[i], dependencies, count);
        check(rc == 0, "Unable to add module class dependency");
    }

    // Main function dependencies.
    if(node->module.main_function != NULL) {
        rc = eql_ast_node_get_dependencies(node->module.main_function, dependencies, count);
...
int eql_ast_module_validate(eql_ast_node *node, eql_module *module)
{
    ...
    uint32_t i;
    for(i=0; i<node->module.class_count; i++) {
        rc = eql_ast_node_validate(node->module.classes[i], module);
        check(rc == 0, "Unable to validate module class");
    }

    // Validate main function.
    if(node->module.main_function != NULL) {
        rc = eql_ast_node_validate(node->module.main_function, module);
