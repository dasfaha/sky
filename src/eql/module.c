...
void eql_module_free(eql_module *module)
{
    if(module) {
        ...
        eql_module_free_ast_modules(module);
...
int eql_module_add_ast_module(eql_module *module, eql_ast_node *ast_module)
{
    ...
    check(ast_module->type == EQL_AST_TYPE_MODULE, "Node type must be 'module'");
    
    // Append ast module to compilation module.
    module->ast_module_count++;
    module->ast_modules = realloc(module->ast_modules, sizeof(eql_ast_node*) * module->ast_module_count);
                             ...
                             eql_ast_node **ret)
{
    ...
    *ret = NULL;
    ...
    for(i=0; i<module->ast_module_count; i++) {
        eql_ast_node *ast_module = module->ast_modules[i];
        
        // Search module for class.
        rc = eql_ast_module_get_class(ast_module, name, ret);
                                      ...
                                      eql_ast_node **ret)
{
    ...
    *ret = NULL;
    ...
    for(i=0; i<module->ast_module_count; i++) {
        ...
        rc = eql_ast_module_get_template_class(ast_module, type_ref, ret);
...
int eql_module_process_dynamic_classes(eql_module *module)
{
    ...
    for(i=0; i<module->ast_module_count; i++) {
        ...
        for(j=0; j<ast_module->module.class_count; j++) {
            eql_ast_node *class = ast_module->module.classes[j];
            
            // Retrieve 'Dynamic' metatag from class.
            struct tagbstring dynamic_metadata_name = bsStatic("Dynamic");
            eql_ast_node *dynamic_metadata = NULL;
            rc = eql_ast_class_get_metadata_node(class, &dynamic_metadata_name, &dynamic_metadata);
            ...
            if(dynamic_metadata != NULL) {
                rc = eql_compiler_process_dynamic_class(module->compiler, class);
...
int eql_module_process_templates(eql_module *module)
{
    ...
    eql_array *sources = eql_array_create(); check_mem(sources);

    // If there is a main function then add it at the beginning.
    eql_ast_node *main_function = NULL;
    rc = eql_module_get_ast_main_function(module, &main_function);
    check(rc == 0, "Unable to retrieve main function");
    
    if(main_function != NULL) {
        rc = eql_array_push(sources, main_function);
    ...
    for(i=0; i<module->ast_module_count; i++) {
        ...
        for(j=0; j<ast_module->module.class_count; j++) {
            ...
            if(class->class.template_var_count == 0) {
                rc = eql_array_push(sources, class);
    ...
    eql_ast_node **type_refs = NULL;
    uint32_t type_ref_count = 0;
    for(i=0; i<sources->length; i++) {
        eql_ast_node *source = sources->elements[i];
        
        // Retrieve list of type references.
        type_refs = NULL;
        type_ref_count = 0;
        rc = eql_ast_node_get_type_refs(source, &type_refs, &type_ref_count);
        check(rc == 0, "Unable to retrieve type refs for node");
        
        // Loop over type refs and generate templates.
        for(j=0; j<type_ref_count; j++) {
            eql_ast_node *type_ref = type_refs[j];
            eql_ast_node *class = NULL;
            rc = eql_module_generate_template_type(module, type_ref, &class);
            ...
            rc = eql_ast_type_ref_flatten(type_ref);
            check(rc == 0, "Unable to flatten type ref");
        }
        
        // Clean up.
        eql_ast_node_type_refs_free(&type_refs, &type_ref_count);
    }

    eql_array_free(sources);
    return 0;
...
{
    int rc;
    eql_ast_template *template = NULL;
    check(module != NULL, "Module required");
    check(type_ref != NULL, "Type reference required");
    check(type_ref->type == EQL_AST_TYPE_TYPE_REF, "Node type must be 'type ref'");
    ...
    rc = eql_ast_type_ref_get_full_name(type_ref, &generated_class_name);
    check(rc == 0, "Unable to calcualte generated class name");
    
    // Only generate the class if it doesn't exist.
    eql_ast_node *generated_class = NULL;
    rc = eql_module_get_ast_class(module, generated_class_name, &generated_class);
    check(rc == 0, "Unable to search for generated class");
    
    if(generated_class == NULL) {
        // Find the type in the module.
        eql_ast_node *template_class = NULL;
        rc = eql_module_get_ast_template_class(module, type_ref, &template_class);
        check(rc == 0, "Unable to retrieve class");
        check(template_class != NULL, "Unable to retrieve class for generation");
    
        // Copy the class.
        rc = eql_ast_node_copy(template_class, &generated_class);
        ...
        eql_ast_class_free_template_vars(generated_class);
    
        // Create template & apply the template.
        template = eql_ast_template_create(template_class, type_ref);
        check_mem(template);
        rc = eql_ast_template_apply(template, generated_class);
        check(rc == 0, "Unable to apply template to generated class");
    
        // Add the generated class to the same module as the template.
        rc = eql_ast_module_add_class(template_class->parent, generated_class);
        check(rc == 0, "Unable to add generated class to module");
    
        eql_ast_template_free(template);
    }
...
void eql_module_free_ast_modules(eql_module *module)
{
    if(module != NULL) {
        uint32_t i;
        for(i=0; i<module->ast_module_count; i++) {
            eql_ast_node_free(module->ast_modules[i]);
                         ...
                         bstring message)
{
    ...
    eql_error *err = eql_error_create(); check_mem(err);
...
void eql_module_free_errors(eql_module *module)
{
    if(module != NULL) {
        ...
        for(i=0; i<module->error_count; i++) {
            eql_error_free(module->errors[i]);
...
int eql_module_ast_dump_stderr(eql_module *module)
{
    ...
    for(i=0; i<module->ast_module_count; i++) {
        rc = eql_ast_node_dump_stderr(module->ast_modules[i]);
