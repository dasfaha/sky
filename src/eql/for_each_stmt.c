...
int eql_ast_for_each_stmt_validate_enumerable_enumerator(eql_ast_node *node,
    eql_module *module);
...
int eql_ast_for_each_stmt_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_for_each_stmt_create(NULL, NULL, NULL);
    check_mem(clone);

    rc = eql_ast_node_copy(node->for_each_stmt.var_decl, &clone->for_each_stmt.var_decl);
    check(rc == 0, "Unable to copy var decl");
    if(clone->for_each_stmt.var_decl) clone->for_each_stmt.var_decl->parent = clone;
    
    rc = eql_ast_node_copy(node->for_each_stmt.enumerator, &clone->for_each_stmt.enumerator);
    check(rc == 0, "Unable to copy enumerator");
    if(clone->for_each_stmt.enumerator) clone->for_each_stmt.enumerator->parent = clone;
    
    rc = eql_ast_node_copy(node->for_each_stmt.block, &clone->for_each_stmt.block);
                                  ...
                                  LLVMValueRef *value)
{
    ...
    rc = eql_ast_node_get_type_name(node->for_each_stmt.enumerator, module, &enumerator_type_name);
                                        ...
                                        uint32_t *count)
{
    ...
    rc = eql_ast_node_get_type_refs(node->for_each_stmt.var_decl, type_refs, count);
    check(rc == 0, "Unable to add variable declaration type refs");

    // Block type refs.
    rc = eql_ast_node_get_type_refs(node->for_each_stmt.block, type_refs, count);
                                           ...
                                           uint32_t *count)
{
    ...
    rc = eql_ast_node_get_dependencies(node->for_each_stmt.var_decl, dependencies, count);
    check(rc == 0, "Unable to add variable declaration dependency");

    // Block dependencies.
    rc = eql_ast_node_get_dependencies(node->for_each_stmt.block, dependencies, count);
...
int eql_ast_for_each_stmt_validate(eql_ast_node *node, eql_module *module)
{
    ...
    rc = eql_ast_for_each_stmt_validate_enumerable_enumerator(node, module);
    check(rc == 0, "Unable to validate enumerable enumerator");

    // Validate variable declaration.
    if(node->for_each_stmt.var_decl != NULL) {
        rc = eql_ast_node_validate(node->for_each_stmt.var_decl, module);
        check(rc == 0, "Unable to validate for each variable declaration");
    }
    
    // Validate enumerator.
    if(node->for_each_stmt.enumerator != NULL) {
        rc = eql_ast_node_validate(node->for_each_stmt.enumerator, module);
        check(rc == 0, "Unable to validate for each statement enumerator");
    }
    
    // Validate block.
    if(node->for_each_stmt.block != NULL) {
        rc = eql_ast_node_validate(node->for_each_stmt.block, module);
                                                         ...
                                                         eql_module *module)
{
    ...
    // Load the enumerator type.
    eql_ast_node *enumerator_type = NULL;
    rc = eql_ast_node_get_type(node->for_each_stmt.enumerator, module, &enumerator_type);
    check(rc == 0 && enumerator_type != NULL, "Unable to find enumerator type");

    // Make sure it's not a built in.
    if(eql_is_builtin_type(enumerator_type)) {
    ...
    if(!msg) {
        ...
        rc = eql_module_get_ast_class(module, enumerator_type->type_ref.name, &class);
        check(rc == 0, "Unable to retrieve class from module");
        
        // Retrieve enumerable metadata from class.
        eql_ast_node *enumerable_metadata = NULL;
        rc = eql_ast_class_get_metadata_node(class, &enumerable_metadata_name, &enumerable_metadata);
