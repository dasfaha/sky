...
int eql_ast_staccess_codegen_property(eql_ast_node *node, eql_module *module,
    LLVMValueRef *value);

int eql_ast_staccess_codegen_method(eql_ast_node *node, eql_module *module,
    LLVMValueRef *value);
                                      ...
                                      unsigned int arg_count)
{
    ...
    // Copy arguments.
    if(arg_count > 0) {
        node->staccess.args = malloc(sizeof(eql_ast_node*) * arg_count);
...
void eql_ast_staccess_free(eql_ast_node *node)
{
    ...
    for(i=0; i<node->staccess.arg_count; i++) {
        eql_ast_node_free(node->staccess.args[i]);
...
int eql_ast_staccess_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_staccess_create(node->staccess.type, NULL, node->staccess.member_name, NULL, 0);
    check_mem(clone);

    rc = eql_ast_node_copy(node->staccess.var_ref, &clone->staccess.var_ref);
    ...
    for(i=0; i<clone->staccess.arg_count; i++) {
        rc = eql_ast_node_copy(node->staccess.args[i], &clone->staccess.args[i]);
                             ...
                             LLVMValueRef *value)
{
    ...
    switch(node->staccess.type) {
        case EQL_AST_STACCESS_TYPE_PROPERTY: {
            rc = eql_ast_staccess_codegen_property(node, module, value);
            check(rc == 0, "Unable to codegen property access");
            break;
        }
        case EQL_AST_STACCESS_TYPE_METHOD: {
            rc = eql_ast_staccess_codegen_method(node, module, value);
                                      ...
                                      LLVMValueRef *value)
{
    ...
    check(node != NULL, "Node required");
    check(node->type == EQL_AST_TYPE_STACCESS, "Node type expected to be 'struct member access'");
    check(node->staccess.type == EQL_AST_STACCESS_TYPE_PROPERTY, "Access type expected to be 'property'");
    check(module != NULL, "Module required");
    check(module->llvm_function != NULL, "Not currently in a function");
                                    ...
                                    LLVMValueRef *value)
{
    ...
    check(node != NULL, "Node required");
    check(node->type == EQL_AST_TYPE_STACCESS, "Node type expected to be 'struct member access'");
    check(node->staccess.type == EQL_AST_STACCESS_TYPE_METHOD, "Access type expected to be 'method'");
    check(module != NULL, "Module required");
    check(module->llvm_function != NULL, "Not currently in a function");
    ...
    rc = eql_ast_node_get_var_pointer(node->staccess.var_ref, module, &caller_pointer);
    ...
    rc = eql_ast_node_get_type_name(node->staccess.var_ref, module, &var_ref_type_name);
    check(rc == 0 && var_ref_type_name != NULL, "Unable to determine caller type");

    // If this is a complex type then dereference first.
    if(eql_llvm_is_double_pointer_type(LLVMTypeOf(caller_pointer))) {
    ...
    for(i=0; i<arg_count; i++) {
        rc = eql_ast_node_codegen(node->staccess.args[i], module, &args[i+1]);
...
int eql_ast_staccess_preprocess(eql_ast_node *node, eql_module *module)
{
    ...
    rc = eql_ast_node_preprocess(node->staccess.var_ref, module);
    check(rc == 0, "Unable to preprocess struct access variable reference");

    // Preprocess arguments.
    uint32_t i;
    for(i=0; i<node->staccess.arg_count; i++) {
        rc = eql_ast_node_preprocess(node->staccess.args[i], module);
                              ...
                              eql_ast_node **type_ref)
{
    ...
    if(node->staccess.type == EQL_AST_STACCESS_TYPE_PROPERTY) {
    ...
    // If this is a method access then grab the return type of the method.
        ...
        rc = eql_ast_class_get_method(class_ast, node->staccess.member_name, &method);
...
int eql_ast_staccess_validate(eql_ast_node *node, eql_module *module)
{
    ...
    rc = eql_ast_node_validate(node->staccess.var_ref, module);
    ...
    uint32_t i;
    for(i=0; i<node->staccess.arg_count; i++) {
        rc = eql_ast_node_validate(node->staccess.args[i], module);
...
int eql_ast_staccess_dump(eql_ast_node *node, bstring ret)
{
    ...
    // Append dump.
    char *type_name = (node->staccess.type == EQL_AST_STACCESS_TYPE_PROPERTY ? "property" : "method");
    ...
    uint32_t i;
    for(i=0; i<node->staccess.arg_count; i++) {
        rc = eql_ast_node_dump(node->staccess.args[i], ret);
