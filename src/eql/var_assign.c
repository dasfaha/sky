...
int eql_ast_var_assign_copy(eql_ast_node *node, eql_ast_node **ret)
{
    ...
    eql_ast_node *clone = eql_ast_var_assign_create(NULL, NULL);
    check_mem(clone);

    rc = eql_ast_node_copy(node->var_assign.var_ref, &clone->var_assign.var_ref);
    check(rc == 0, "Unable to copy var ref");
    if(clone->var_assign.var_ref) clone->var_assign.var_ref->parent = clone;
    
    rc = eql_ast_node_copy(node->var_assign.expr, &clone->var_assign.expr);
                                ...
                                LLVMValueRef *value)
{
    ...
    if(node->var_assign.expr->type == EQL_AST_TYPE_NULL_LITERAL) {
...
int eql_ast_var_assign_preprocess(eql_ast_node *node, eql_module *module)
{
    ...
    rc = eql_ast_node_preprocess(node->var_assign.var_ref, module);
    check(rc == 0, "Unable to preprocess variable assignment reference");
    
    // Preprocess expression.
    rc = eql_ast_node_preprocess(node->var_assign.expr, module);
...
int eql_ast_var_assign_validate(eql_ast_node *node, eql_module *module)
{
    ...
    if(node->var_assign.var_ref->type == EQL_AST_TYPE_STACCESS && 
       node->var_assign.var_ref->staccess.type == EQL_AST_STACCESS_TYPE_METHOD)
    {
        msg = bformat("Illegal assignment to a method invocation");
    }
    
    // Validate variable reference.
    rc = eql_ast_node_validate(node->var_assign.var_ref, module);
    check(rc == 0, "Unable to validate variable assignment reference");
    
    // Validate expression.
    rc = eql_ast_node_validate(node->var_assign.expr, module);
                                      ...
                                      uint32_t *count)
{
    ...
    rc = eql_ast_node_get_type_refs(node->var_assign.expr, type_refs, count);
