...
qip_ast_node *qip_ast_float_literal_create(double value)
{
    ...
    node->type = QIP_AST_TYPE_FLOAT_LITERAL;
    node->parent = NULL;
    node->line_no = node->char_no = 0;
    node->generated = false;
    node->float_literal.value = value;
    node->float_literal.type_ref = qip_ast_type_ref_create_cstr("Float");
...
{
    if(node != NULL) {
        qip_ast_type_ref_free(node->float_literal.type_ref);
    }
}
...
int qip_ast_float_literal_copy(qip_ast_node *node, qip_ast_node **ret)
{
    ...
    qip_ast_node *clone = qip_ast_float_literal_create(node->float_literal.value);
    check_mem(clone);
...
//
// Returns 0 if successful, otherwise returns -1.
int qip_ast_float_literal_codegen(qip_ast_node *node,
                                  qip_module *module,
                                  LLVMValueRef *value)
{
...
//
// Returns 0 if successful, otherwise returns -1.
int qip_ast_float_literal_get_type(qip_ast_node *node,
                                   qip_ast_node **type_ref)
{
    check(node != NULL, "Node required");
    check(node->type == QIP_AST_TYPE_FLOAT_LITERAL, "Node type must be 'float literal'");
