...
//
// Returns 0 if successful, otherwise returns -1.
int qip_ast_module_get_var_refs_by_type(qip_ast_node *node, qip_module *module,
                                        bstring type_name, qip_array *array)
{
    ...
    uint32_t i;
    for(i=0; i<node->module.class_count; i++) {
        rc = qip_ast_node_get_var_refs_by_type(node->module.classes[i], module, type_name, array);
        check(rc == 0, "Unable to add module class var refs by type");
    }

    rc = qip_ast_node_get_var_refs_by_type(node->module.main_function, module, type_name, array);
    check(rc == 0, "Unable to add main function var refs");
