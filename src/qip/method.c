...
{
    if(node->method.function) qip_ast_node_free(node->method.function);
    node->method.function = NULL;

    unsigned int i;
    for(i=0; i<node->method.metadata_count; i++) {
        qip_ast_node_free(node->method.metadatas[i]);
...
int qip_ast_method_copy(qip_ast_node *node, qip_ast_node **ret)
{
    ...
    qip_ast_node *clone = qip_ast_method_create(node->method.access, NULL);
    check_mem(clone);

    rc = qip_ast_node_copy(node->method.function, &clone->method.function);
    ...
    for(i=0; i<clone->method.metadata_count; i++) {
        rc = qip_ast_node_copy(node->method.metadatas[i], &clone->method.metadatas[i]);
...
//
// Returns 0 if successful, otherwise returns -1.
int qip_ast_method_add_metadata(qip_ast_node *node, qip_ast_node *metadata)
{
    // Validate.
    check(node != NULL, "Node required");
    check(node->type == QIP_AST_TYPE_METHOD, "Node type must be 'method'");
    check(metadata != NULL, "Metadata required");
    
    // Append metadata.
    node->method.metadata_count++;
    node->method.metadatas = realloc(node->method.metadatas, sizeof(qip_ast_node*) * node->method.metadata_count);
                                 ...
                                 unsigned int metadata_count)
{
    ...
    for(i=0; i<metadata_count; i++) {
        int rc = qip_ast_method_add_metadata(node, metadatas[i]);
                           ...
                           LLVMValueRef *value)
{
    ...
    unsigned int i;
    for(i=0; i<node->method.metadata_count; i++) {
        rc = qip_ast_node_codegen(node->method.metadatas[i], module, NULL);
        check(rc == 0, "Unable to codegen method metadata");
    }

    // Codegen function.
    if(node->method.function != NULL) {
        rc = qip_ast_node_codegen(node->method.function, module, value);
...
//
// Returns 0 if successful, otherwise returns -1.
int qip_ast_method_codegen_forward_decl(qip_ast_node *node,
                                        qip_module *module)
{
    int rc;
    check(node != NULL, "Node required");
    ...
    unsigned int i;
    for(i=0; i<node->method.metadata_count; i++) {
        rc = qip_ast_node_codegen_forward_decl(node->method.metadatas[i], module);
        check(rc == 0, "Unable to codegen method metadata forward declaration");
    }

    // Codegen function.
    if(node->method.function) {
        rc = qip_ast_node_codegen_forward_decl(node->method.function, module);
...
int qip_ast_method_validate(qip_ast_node *node, qip_module *module)
{
    ...
    unsigned int i;
    for(i=0; i<node->method.metadata_count; i++) {
        rc = qip_ast_node_validate(node->method.metadatas[i], module);
        check(rc == 0, "Unable to validate method metadata");
    }

    // Validate variable declaration
    rc = qip_ast_node_validate(node->method.function, module);
...
int qip_ast_method_generate_this_farg(qip_ast_node *node)
{
    ...
    check(node->parent->type == QIP_AST_TYPE_CLASS, "Node parent type must be 'class'");

    qip_ast_node *function = node->method.function;

    // Only generate 'this' arg if the function exists.
    if(function != NULL) {
        // Generate type ref from class.
        qip_ast_node *type_ref = NULL;
        rc = qip_ast_class_generate_type_ref(node->parent, &type_ref);
        check(rc == 0, "Unable to generate class type ref");
    
        // Create 'this' variable declaration.
        struct tagbstring THIS = bsStatic("this");
        qip_ast_node *var_decl = qip_ast_var_decl_create(type_ref, &THIS, NULL);
        ...
        function->function.args = realloc(function->function.args, sizeof(qip_ast_node*) * function->function.arg_count);
        check_mem(function->function.args);
        memmove(function->function.args+1, function->function.args, sizeof(qip_ast_node*) * (function->function.arg_count-1));
                                 ...
                                 uint32_t *count)
{
    ...
    if(node->method.function != NULL) {
        rc = qip_ast_node_get_type_refs(node->method.function, type_refs, count);
...
//
// Returns 0 if successful, otherwise returns -1.
int qip_ast_method_get_var_refs_by_type(qip_ast_node *node, qip_module *module,
                                        bstring type_name, qip_array *array)
{
    ...
    rc = qip_ast_node_get_var_refs_by_type(node->method.function, module, type_name, array);
    check(rc == 0, "Unable to add method function var refs");
                                    ...
                                    uint32_t *count)
{
    ...
    if(node->method.function != NULL) {
        rc = qip_ast_node_get_dependencies(node->method.function, dependencies, count);
...
int qip_ast_method_dump(qip_ast_node *node, bstring ret)
{
    ...
    if(node->method.function != NULL) {
        rc = qip_ast_node_dump(node->method.function, ret);
