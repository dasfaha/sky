...
qip_ast_node *qip_ast_null_literal_create()
{
    ...
    node->type = QIP_AST_TYPE_NULL_LITERAL;
    node->parent = NULL;
    node->line_no = node->char_no = 0;
    node->generated = false;
    node->null_literal.type_ref = qip_ast_type_ref_create_cstr("Ref");
...
{
    if(node != NULL) {
        qip_ast_type_ref_free(node->null_literal.type_ref);
    }
}
...
int qip_ast_null_literal_copy(qip_ast_node *node, qip_ast_node **ret)
{
    ...
    qip_ast_node *clone = qip_ast_null_literal_create();
    check_mem(clone);
...
//
// Returns 0 if successful, otherwise returns -1.
int qip_ast_null_literal_codegen(qip_ast_node *node,
                                 qip_module *module,
                                 LLVMValueRef *value)
{
    check(node != NULL, "Node required");
    check(node->type == QIP_AST_TYPE_NULL_LITERAL, "Node type must be 'null literal'");
