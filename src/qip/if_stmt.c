...
//
// Returns 0 if successful, otherwise returns -1.
int qip_ast_if_stmt_get_var_refs_by_type(qip_ast_node *node, qip_module *module,
                                         bstring type_name, qip_array *array)
{
    ...
    uint32_t i;
    for(i=0; i<node->if_stmt.block_count; i++) {
        rc = qip_ast_node_get_var_refs_by_type(node->if_stmt.blocks[i], module, type_name, array);
        check(rc == 0, "Unable to add if block var refs by type");
    }

    rc = qip_ast_node_get_var_refs_by_type(node->if_stmt.else_block, module, type_name, array);
    check(rc == 0, "Unable to add else block var refs by type");
